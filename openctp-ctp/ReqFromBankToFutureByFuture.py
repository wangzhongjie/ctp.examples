...
# @File:    ReqFromBankToFutureByFuture.py
# @Time:    06/06/2024 21:05
# @Author:  Jedore
...
class CTdSpi(CTdSpiBase):
    ...
    def req(self):
        """ 期货发起银行资金转期货请求
        doc: https://ctpdoc.jedore.top/6.7.9/JYJK/CTHOSTFTDCTRADERSPI/REQFROMBANKTOFUTUREBYFUTURE/
        """
        # SimNow 不支持，需要实盘测验
        self.print("期货发起银行资金转期货请求")
        ...
        self._check_req(req, self._api.ReqFromBankToFutureByFuture(req, 0))

    def OnRspFromBankToFutureByFuture(self, pReqTransfer: tdapi.CThostFtdcReqTransferField,
    ...
    def OnRtnFromBankToFutureByBank(self, pRspTransfer: tdapi.CThostFtdcRspTransferField):
        """ 期货发起银行资金转期货请求通知 """
        self._print_rtn(pRspTransfer, "期货发起银行资金转期货请求通知")
